...
public class Kardex {
	...
	private static final String[] TIPOS_METODOS = { "PEPS - Primero en entrar, primero en salir",
	...
	private static final String[] ANIOS = { "2016", "2017", "2018", "2019", "2020", "2021", "2022", "2023", "2024",
			"2025" };
	...
	public String[] nombresKardexCreados() {
		...
		for (int i = 0; i < archivos.length; i++) {
			...
			nombre = nombre.length() < 75 ? nombre
					: nombre.substring(0, 52) + "..." + nombre.substring(nombre.length() - 20, nombre.length());
			datos[i] = nombre;
		}
		return datos;
	}
	...
	private boolean hayRayasPiso(String[] datos) {
		if (datos.length != 9) // Verificacion extra no necesaria, son 9 datos que el usuario digita al crear
								// Kardex
			return true;
	...
	public void addDatos(String[] datos) {
		...
		if (valorU != 0) {
			...
			Elemento agregar = new Elemento(dia, descripcion, valorU, cantidadEntrada, 0, 0, 0, 0, 0);
			elementos.add(agregar);

		} else if (valorU == 0) {

			if (cantidadS != 0) {
				Elemento agregar = new Elemento(dia, descripcion, valorU, 0, 0, cantidadS, 0, 0, 0);
				elementos.add(agregar);
			} else {
				Elemento agregarL = search(dia);
				int cant = agregarL.getCantidadSalida() * -1;
				Elemento agreg = new Elemento(agregarL.getDia(), agregarL.getDescripcion(), 0, 0, 0, cant, 0 ,0, 0);
				elementos.add(agreg);
			}
